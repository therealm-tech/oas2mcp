...
| oas2mcp.auth.traceClaims | list | `[]` | JWT claim names copied into the per-call tracing log as a `jwt.claims` field (e.g. `sub`, `email`, `tenant_id`), for observability. Logged only, never a metric label. Needs `roleMapper` (claims come from the verified JWT). Maps to `TRACE_CLAIMS`. |
